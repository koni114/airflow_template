...
def get_config_from_env(included_str=""):
    """ make dict type object from environment variable.
        Transforms the environment variable value into a dict form when the prefix string is included.
    """
    from collections import defaultdict
    config_env_dict = defaultdict(dict)
    for key, value in os.environ.items():
        if included_str in key:
            config_env_dict[key.lower()] = value
    return config_env_dict
...
def set_env_minIO(conf: dict) -> None:
    ...
    for key, value in conf.items():
        if "minio_" in key:
            os.environ[key.replace("minio_", "").upper()] = value
        
env_minio_config = get_config_from_env(included_str="MINIO")
env_mlflow_config = get_config_from_env(included_str="MLFLOW")
config = get_config_from_yaml()

env_config = add_update_dict(env_minio_config, env_mlflow_config)
config = add_update_dict(env_config, config)